...
export function SumberPengajuanBadge({ sumber, className }: { sumber: SumberPengajuan; className?: string }) {
  const map: Record<SumberPengajuan, { label: string; cls: string }> = {
    PUBLIC:        { label: "Kode Akses",    cls: "bg-[oklch(0.94_0.06_150)] text-[oklch(0.36_0.10_152)]" },
    PW_DASHBOARD:  { label: "Login Digdaya", cls: "bg-[oklch(0.93_0.06_240)] text-[oklch(0.34_0.10_240)]" },
    PC_DASHBOARD:  { label: "Login Digdaya", cls: "bg-[oklch(0.93_0.06_240)] text-[oklch(0.34_0.10_240)]" },
  };
  const m = map[sumber];
  return (
