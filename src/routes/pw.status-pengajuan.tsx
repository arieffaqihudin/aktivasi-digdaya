  ...
  return (
    <div>
      <PageHeader
        ...
        count={total}
        breadcrumb={[{ label: "PW", to: "/pw" }, { label: "Aktivasi Organisasi" }, { label: "Status Pengajuan" }]}
        subtitle="Semua pengajuan dari dashboard PW Anda."
      />
      ...
      <FilterBar>
        <SelectFilter value={tipeFilter} onChange={setTipeFilter} placeholder="Tipe Organisasi" options={[["all","Semua Tipe"],["PC","PC"],["MWC","MWC"],["Lembaga PW","Lembaga PW"]]} />
      ...
      <DataTable>
        ...
        <tbody>
            ...
            <TR key={r.ticketId}>
              ...
              <TD className="text-right pr-6">
                <div className="flex justify-end gap-1">
                  <Link to="/pw/status-pengajuan/$ticketId" params={{ ticketId: r.ticketId }}>
                    <RowAction title="Lihat detail"><Eye className="h-4 w-4" /></RowAction>
                  </Link>
                  {r.status === "PerluPerbaikan" && (
                    <Link to="/pw/status-pengajuan/$ticketId/revisi" params={{ ticketId: r.ticketId }}>
          ...
          {paged.length === 0 && <EmptyRow colSpan={8}>Belum ada pengajuan.</EmptyRow>}
        </tbody>
      </DataTable>
