...
type TypeFilter = "ALL" | "Lembaga PW";
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Pantau status pengajuan aktivasi Lembaga PW.</p>
        </div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.ticketId} className="border-t border-border hover:bg-primary/5 transition-colors">
                ...
                <td className="px-4 py-3 text-foreground max-w-[260px]"><span className="line-clamp-1">{r.namaOrg}</span></td>
                ...
                <td className="px-4 py-3"><RowActions row={r} /></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function MobileList({ rows }: { rows: Row[] }) {
  return (
...
function RowActions({ row, mobile }: { row: Row; mobile?: boolean }) {
  return (
  ...
  return (
    <div className="rounded-xl border border-dashed border-border bg-card p-10 text-center">
      ...
      <p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">
        Pengajuan Lembaga PW yang Anda kirim akan muncul di sini.
      </p>
      <Link
        to="/pw/daftarkan"
